...
from ..core.models import CallSpec, ProgramSpec, SimulationResult
...
class SessionContext:
    ...
    def __enter__(self) -> Session:
        self.session = self._service.start_session(
            self._program_id,
    ...
    def __exit__(self, exc_type, exc, tb) -> bool:
        if self.session is None:
            return False
        if self.session.session_id in self._service.sessions:
            self._service.end_session(self.session.session_id)
        if self._drain_on_exit and exc_type is None:
            self._service.drain()
        return False
...
class AutellixService:
    ...
    def session(
        ...
        *,
        ...
        return SessionContext(self, program_id, arrival_time, drain_on_exit)
    ...
    def chat_completion(
        ...
        **metadata: Any,
        ...
        call = self.submit_call(
            ...
            thread_id=thread_id,
            metadata={
                **(framework_metadata or {}),
                **({"request_metadata": metadata} if metadata else {}),
            },
        )
